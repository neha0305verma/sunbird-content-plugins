 ...
 * @class org.ekstep.questionunitmcq:feedbackQuestionFormController
 * Jagadish P<jagadish.pujari@tarento.com>
 */
angular.module('feedbackApp', ['org.ekstep.question']).controller('feedbackQuestionFormController', ['$scope', '$rootScope', 'questionServices', '$timeout', function ($scope, rootScope, questionServices, $timeout) {
  ...
  questionInput.on('change', function () {
    $scope.feedbackFormData.question.text = this.getData();
  });
  ...
  $scope.init = function () {
    $scope.mcqPluginInstance = org.ekstep.pluginframework.pluginManager.getPluginManifest("org.ekstep.questionunit.feedback")
    EventBus.listeners['org.ekstep.questionunit.feedback:validateform'] = [];
    ecEditor.addEventListener('org.ekstep.questionunit.feedback:validateform', function (event, callback) {
      var validationRes = $scope.formValidation();
      callback(validationRes.isValid, validationRes.formData);
    }, $scope);
    EventBus.listeners['org.ekstep.questionunit.feedback:editquestion'] = [];
    ecEditor.addEventListener('org.ekstep.questionunit.feedback:editquestion', $scope.editMcqQuestion, $scope);
    ecEditor.dispatchEvent("org.ekstep.questionunit:ready");
    $scope.BindCkeditor();
  }
  $scope.editMcqQuestion = function (event, data) {
    var qdata = data.data;
    $scope.feedbackFormData.question = qdata.question;
    $scope.feedbackFormData.options = qdata.options;
    $scope.editMedia = qdata.media;
    var opLength = qdata.length;
    if (opLength > 2) {
      for (var j = 2; j < opLength; j++) {
        $scope.feedbackFormData.options.push({
          'text': '',
          'image': '',
          'audio': '',
          'audioName': '',
    ...
    if ($scope.feedbackFormData.options.length < 2) {
      $scope.feedbackFormData.options.splice(2, 1);
    }
    $scope.$safeApply();
  }
  $scope.addAnswerField = function () {
    ...
    if ($scope.feedbackFormData.options.length < 8) $scope.feedbackFormData.options.push(option);
    $scope.BindCkeditor();
  }
  $scope.formValidation = function () {
    ...
    var valid = false;
    // debugger;
    var formValid = $scope.feedbackForm.$valid && $scope.feedbackFormData.options.length > 1;
    $scope.submitted = true;
    if (!($scope.feedbackFormData.question.text.length || $scope.feedbackFormData.question.image.length || $scope.feedbackFormData.question.audio.length)) {
      $('.questionTextBox').addClass("ck-error");
    } else {
      $('.questionTextBox').removeClass("ck-error");
    }
    if (!_.isUndefined($scope.selectedOption)) {
      _.each($scope.feedbackFormData.options, function (k, v) {
        $scope.feedbackFormData.options[v].isCorrect = false;
        console.log('checking');
      });
      valid = true;
      console.log('debug');
      $scope.feedbackFormData.options[$scope.selectedOption].isCorrect = true;
    } else {
      _.each($scope.feedbackFormData.options, function (k, v) { // eslint-disable-line no-unused-vars
        // if (k.isCorrect) {
        //   valid = true;
        // } else {
          valid = true;
    ...
    $scope.feedbackFormData.media = $scope.editMedia;
    //check if audio is their then add audio icon in media array
    if ($scope.feedbackFormData.media.length > 0) $scope.addDefaultMedia();
    var formConfig = {};
    formConfig.formData = $scope.feedbackFormData;
    if (formValid && opSel) {
  ...
  $scope.deleteAnswer = function (id) {
    if (id >= 0) $scope.feedbackFormData.options.splice(id, 1);
  ...
  $scope.addDefaultMedia = function () {
    ...
    addAllMedia.forEach(function (obj) {
      $scope.feedbackFormData.media.push(obj);
    })
  }
  /**
  ...
  $scope.addMedia = function (type, index, mediaType) {
    ...
    mediaObject.callback = function (data) {
      ...
      if (type == 'q') {
        ...
        $scope.feedbackFormData.question[mediaType] = org.ekstep.contenteditor.mediaManager.getMediaOriginURL(data.assetMedia.src);
        data.assetMedia.type == 'audio' ? $scope.feedbackFormData.question.audioName = data.assetMedia.name : '';
        $scope.questionMedia[mediaType] = media;
      } else {
        telemetryObject.target.id = 'questionunit-mcq-option-add-' + mediaType;
        $scope.feedbackFormData.options[index][mediaType] = org.ekstep.contenteditor.mediaManager.getMediaOriginURL(data.assetMedia.src);
        data.assetMedia.type == 'audio' ? $scope.feedbackFormData.options[index].audioName = data.assetMedia.name : '';
  ...
  $scope.deleteMedia = function (type, index, mediaType) {
    ...
    if (type == 'q') {
      $scope.feedbackFormData.question[mediaType] = '';
      delete $scope.questionMedia[mediaType];
    } else {
      $scope.feedbackFormData.options[index][mediaType] = '';
  ...
  $scope.deleteHint = function (id) {
    if (id == 'q') {
      $scope.qHint = false;
      $scope.feedbackFormData.question.hint = '';
    } else {
      $scope.oHint[id] = false;
      $scope.feedbackFormData.options[id].hint = '';
    }
  }
  /**
  ...
  $scope.ckEditorEventHandler = function (index) {
    ...
    CKEDITOR.instances[optionelement.id].setData($scope.feedbackFormData.options[index].text);
    optionInput.on('change', function () {
      //on changes get index id and assign to model
      var id = parseInt(this.name.split("mcqoptions_")[1]);
      $scope.feedbackFormData.options[id].text = CKEDITOR.instances[this.name].getData();
      $scope.$safeApply();
    });
